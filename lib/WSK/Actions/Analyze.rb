...
module WSK
  ...
  module Actions
    ...
    class Analyze
      ...
      def execute2(iInputData, oOutputData)
        lMaxValues = nil
        lMinValues = nil
        lSumValues = nil
        lAbsSumValues = nil
        lSquareSumValues = nil
        ...
        iInputData.eachBuffer do |iBuffer, iNbrSamples, iNbrChannels|
          lIdxFirstSample = 0
          if (lMaxValues == nil)
            # Initialize the values with the first sample
            lMaxValues = []
            lMinValues = []
            lSumValues = []
            lAbsSumValues = []
            ...
            iNbrChannels.times do |iIdxChannel|
              lValue = iBuffer[iIdxChannel]
              lMaxValues << lValue
              lMinValues << lValue
              lSumValues << lValue
              lAbsSumValues << lValue.abs
              lSquareSumValues << lValue*lValue
            end
            lIdxFirstSample = 1
          end
          (lIdxFirstSample..iNbrSamples-1).each do |iIdxSample|
            iNbrChannels.times do |iIdxChannel|
              lValue = iBuffer[iIdxSample*iNbrChannels+iIdxChannel]
              if (lValue > lMaxValues[iIdxChannel])
                lMaxValues[iIdxChannel] = lValue
              end
              if (lValue < lMinValues[iIdxChannel])
                lMinValues[iIdxChannel] = lValue
              end
              lSumValues[iIdxChannel] += lValue
              lAbsSumValues[iIdxChannel] += lValue.abs
              lSquareSumValues[iIdxChannel] += lValue*lValue
            end
          end
